...
while dif > tol & its<maxits
  ...
  [V1_its,a1_its]=max(U1mat_1 + beta*repmat(V*Pi(1,:)',1,na));
  [V2_its,a2_its]=max(U1mat_2 + beta*repmat(V*Pi(2,:)',1,na));
...
hold on
plot(agrid,V(:,2),'b');
title({'{\bf\fontsize{14} Value function}'; '(CRRA Utility and Liquidity Constraint'})
xlabel('a')
ylabel('V')
...
hold on
plot(agrid,V(:,2),'b');
title({'{\bf\fontsize{14} Value function}'; '(CRRA Utility and Precautionary Savings'})
xlabel('a')
ylabel('V')
legend('High-income', 'Low-income')
hold off
print -dpdf pe1_1.eps
